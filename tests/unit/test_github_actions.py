...
class TestValidateWorkflowCompatibility:
    ...
    def test_compatible_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Create a workflow with the current (v3 build-once) version marker
...
"""
            ...
            # v3 marker matches the current workflow version -> compatible
            assert is_valid is True
            assert details["current_workflow"] == "3.0"
...
class TestWorkflowV3BuildOnce:
    """v3 build-once workflow (RFC 0001 D4 / W2.5): canonical trigger, single
    build job, per-classifier alias step, valid YAML."""

    def _wf(self, **cfg):
        return generate_build_and_push_workflow(
            repo_name="myrepo",
            mlserver_source_url="https://github.com/org/mlserver.git",
            **cfg,
        )
    ...
    def _on_block(self, parsed):
        # YAML 1.1 parses the bare key 'on' as boolean True.
        return parsed.get("on", parsed.get(True))

    def test_workflow_is_valid_yaml_ghcr(self):
    ...
    def test_trigger_matches_canonical_tag_not_legacy(self):
        ...
        assert "*/v*" in tags
        # A canonical release tag fires the workflow...
        assert any(fnmatch.fnmatch("sentiment/v1.0.0", pat) for pat in tags)
    ...
    def test_single_build_job(self):
        parsed = self._parse(self._wf())
        assert list(parsed["jobs"].keys()) == ["build-and-push"]
    ...
    def test_per_classifier_alias_step_present(self):
        ...
        assert any("Alias" in n for n in names)
        # The alias is derived from the pushed commit image, keyed by classifier+version
        assert "${CLASSIFIER}-v${VERSION}" in run_blocks
        assert "${CLASSIFIER}-latest" in run_blocks

    def test_smoke_test_uses_classifier_env(self):
        parsed = self._parse(self._wf())
        steps = parsed["jobs"]["build-and-push"]["steps"]
        run_blocks = "\n".join(s.get("run", "") for s in steps)
        # The commit image is smoke-tested by selecting a classifier at run time
        assert "MLSERVER_CLASSIFIER=$CLASSIFIER" in run_blocks

    def test_parse_step_uses_canonical_split(self):
        parsed = self._parse(self._wf())
        steps = parsed["jobs"]["build-and-push"]["steps"]
        run_blocks = "\n".join(s.get("run", "") for s in steps)
        # classifier = before '/v', version = after '/v'
        assert "${TAG_NAME%%/v*}" in run_blocks
        assert "${TAG_NAME##*/v}" in run_blocks

    def test_workflow_version_marker_is_v3(self):
        wf = self._wf()
        assert "Workflow version: 3.0" in wf
