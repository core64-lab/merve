...
**ARCHIVED (2026-07-04):** Issue #1 was fixed — `merve version` works without a
config file (tool-only output; pinned by `tests/unit/test_cli_commands.py::
TestVersionCommand::test_version_human_output`). The referenced `cli_v2.py`
module no longer exists (the CLI is the `mlserver/cli/` package, RFC 0001 D6).
This file is retained for historical reference only; new issues go to the
issue tracker.
